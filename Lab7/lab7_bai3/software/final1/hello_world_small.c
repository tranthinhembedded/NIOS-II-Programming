...
void Key_Handler(void) {
    ...
    if (mode == 1) { // Edit time
        if (key1_state & 0x01) { // KEY1: Increment hour
            hh = (hh + 1) % 24;
        }
        if (key2_state & 0x01) { // KEY2: Increment minute
            mm = (mm + 1) % 60;
        }
        if (key3_state & 0x01) { // KEY3: Increment second
            ss = (ss + 1) % 60;
        }
    } else if (mode == 2) { // Edit date
        if (key1_state & 0x01) { // KEY1: Increment day
            dd = (dd % 31) + 1; // Simplified: max 31 days
        }
        if (key2_state & 0x01) { // KEY2: Increment month
            month = (month % 12) + 1;
        }
        if (key3_state & 0x01) { // KEY3: Increment year
            year = (year + 1) % 10000;
        }
    }
}
...
int main() {
    ...
    alt_ic_isr_register(0, SW_IRQ, Switch_IRQ_Handler, NULL, NULL);
